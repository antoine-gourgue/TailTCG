...
export type SealedCote = { value: number; source: "cardmarket" | "cardmarket-low" | "tcgplayer" };
...
export async function sealedCotes(products: SealedPriceInput[], client?: SupabaseClient<Database>): Promise<Map<number, SealedCote>> {
  const guide = await fetchGuideRefs(products.map((p) => p.cardmarket_id), client);
  const out = new Map<number, SealedCote>();
  for (const p of products) {
    const cm = p.cardmarket_id != null ? guide.get(p.cardmarket_id) : undefined;
    if (cm) out.set(p.id, { value: cm.value, source: cm.field === "low" ? "cardmarket-low" : "cardmarket" });
